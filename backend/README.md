...
3. В панели Timeweb (или другом хостинге контейнеров) укажите:
   ...
   Если хостинг требует слушать порт из переменной `PORT`, используйте её значение
   вместо `8000` (Docker-образ уже учитывает `PORT`).
   Для healthcheck используйте `GET /health` — эндпоинт уже заложен в контейнерный
   `HEALTHCHECK` и подходит для Timeweb Cloud.
