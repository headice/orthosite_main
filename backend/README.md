...
## Запуск сервера
1. **Без Docker.** Установите зависимости из корня проекта и запустите uvicorn через общий энтрипоинт:
   ```bash
   pip install -r requirements.txt
   uvicorn main:app --host 0.0.0.0 --port 8000
   ```
   Это то же самое, что стартовать из каталога `backend` командой `uvicorn backend.main:app --host 0.0.0.0 --port 8000`. В репозитории лежит `Procfile` с этой командой, чтобы облачные платформы могли запускать проект без Docker.
