...
## Запуск сервера
1. **Без Docker.** Установите зависимости из корня проекта и запускайте FastAPI напрямую из модуля `backend.main` — это путь, который ожидают облачные площадки:
   ```bash
   pip install -r requirements.txt
   uvicorn backend.main:app --host 0.0.0.0 --port 8000
   ```
   При необходимости можно импортировать приложение как `main:app` (корневой файл просто проксирует `backend/main.py`), но основной и рекомендуемый путь — `backend.main:app`. В репозитории лежит `Procfile` с этой командой, чтобы облачные платформы могли запускать проект без Docker.
