...
- Страница `/` — простая форма, которая показывает цену и создает платеж, открывая ссылку подтверждения в новой вкладке.
...
## Быстрый тест из браузера
Запустите сервер (`uvicorn app.main:app --reload --host 0.0.0.0 --port 8000`) и откройте `http://localhost:8000/`.
На странице будет форма:

- текущая цена по календарю;
- поля «Описание платежа» и «Return URL» (по умолчанию `http://localhost:8000/payment-complete`, можно заменить);
- кнопка «Создать платеж», которая вызывает `/payments`, затем открывает `confirmation_url` ЮKassa в новой вкладке.

Ошибки (например, если не заданы `YOOKASSA_SHOP_ID` / `YOOKASSA_SECRET_KEY`) отображаются прямо на странице.
